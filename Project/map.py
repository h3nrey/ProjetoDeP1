...
class Map():
	...
	def check_tiles_collision(self):
		# print("checking tiles collision")
		for tile in self.box_tiles:
			for i, tile2 in enumerate(self.tiles):
				ray_left = pg.draw.line(self.screen, "green", tile.rect.center, (tile.rect.left - 20, tile.rect.center[1]))
				ray_right = pg.draw.line(self.screen, "green", tile.rect.center, (tile.rect.right + 20, tile.rect.center[1]))
				ray_top = pg.draw.line(self.screen, "red", tile.rect.center, (tile.rect.center[0], tile.rect.top - 20))
				ray_bottom = pg.draw.line(self.screen, "blue", tile.rect.center, (tile.rect.center[0], tile.rect.bottom + 20))

				if(ray_left.colliderect(tile2)): tile.rays["left"] = True
				if(ray_right.colliderect(tile2)): tile.rays["right"] = True
				if(ray_top.colliderect(tile2)): tile.rays["top"] = True
				if(ray_bottom.colliderect(tile2)): tile.rays["bottom"] = True
